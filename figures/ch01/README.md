...
第1章「他言語との差分マップ」用の図版2点（`requirements.md` 4節「目次構成表」で
第1章の図版点数は2点と定義済み。GitHub Issue #5対応）。
...
2. **プレースホルダー**（`figure-XX-*-placeholder.svg`）：本番のAI生成画像に
   差し替えるまでの仮画像。単純な枠線・ハッチングと注記のみで構成し、
   FontAwesomeアイコンや絵文字などの装飾は含めない（プレースホルダーである旨を
   混同させないため）。本文からはこのSVGを参照しており、本番画像に差し替える際は
...
|---|---|---|---|---|
| 図1-1 | `figure-01-diff-landscape-spec.md` | `figure-01-diff-landscape-placeholder.svg` | 1.2節「12の差分マップ」表の直後 | 4つの前提言語グループ（Java/C#・JS/TS・Ruby/PHP・Go/Rust）が、12の差分ポイントのどこで最初に躓くかを地図の比喩で示す |
| 図1-2 | `figure-02-reader-routes-spec.md` | `figure-02-reader-routes-placeholder.svg` | 1.8節「読者タイプ別の読み進め方」表の直後 | 4つの読者タイプ（P1〜P4）ごとの推奨章ルートを、路線図（トレイルマップ）の比喩で示す |
